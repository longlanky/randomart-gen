...
export function makeChaoticPalette(rng) {
  const hue = rng.range(0, 360);
  const theme = rng.pick(['dark', 'dark', 'muted', 'paper']);
  const dark = theme === 'dark';
  const offsets = rng.pick([[0, 120, 240, 180], [0, 35, 180, 210], [0, 160, 200, 310]]);
  const colors = offsets.map((offset) => oklchToRgb(rng.range(dark ? 0.55 : 0.4, 0.83), rng.range(0.16, 0.3), hue + offset));
  return {
    theme,
    scheme: 'eclectic',
    background: oklchToRgb(dark ? 0.15 : theme === 'paper' ? 0.94 : 0.6, 0.025, hue),
    dominant: colors[0], support: colors[1], accent: colors[2],
    ink: oklchToRgb(dark ? 0.94 : 0.2, 0.025, hue),
    colors,
  };
}
